...
class ExpenseTracker:
    ...
    def track_buget(self):
        ...
        with open("my_budget.json", "r") as file:
            budget = json.load(file)

        if not budget:
            b_amount = input('Enter the total amount they want to budget for the month: ')
            budget = {
                "amount": b_amount
            }

        total_exp = self.get_total_expense_by_date(now)
        diff = budget['amount'] - total_exp
        if diff < 0:
